...
bool FanetLora::begin(int8_t sck, int8_t miso, int8_t mosi, int8_t ss,int8_t reset, int8_t dio0, int8_t gpio,long frequency,uint8_t outputPower,uint8_t radio, MacAddr addr){
  ...
  bool bRet = fmac.begin(sck, miso, mosi, ss, reset, dio0,gpio,*fa,frequency,outputPower,radio, addr);
...
void FanetLora::sendGroundTracking(trackingData *tData){
  Frame *frm = new Frame(fmac.myAddr);
  frm->type = FRM_TYPE_GROUNDTRACKING;
  frm->payload_length = serialize_GroundTracking(tData,frm->payload);
  frm2txBuffer(frm);
}
