...
bool FanetMac::begin(int8_t sck, int8_t miso, int8_t mosi, int8_t ss,int8_t reset, int8_t dio0,int8_t gpio,Fapp &app,long frequency,uint8_t level,uint8_t radioChip, MacAddr addr)
{
	myApp = &app;
...
void FanetMac::handleTx()
{
	...
	{
#if MAC_debug_mode > 0
		printf("### Problem serialization type 0x%02X: %i removing.\r\n", frm->type, blength);
#endif
